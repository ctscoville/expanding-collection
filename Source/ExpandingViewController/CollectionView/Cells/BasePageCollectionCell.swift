...
extension BasePageCollectionCell {
    ...
    public func setCellPlacement(visibleRect: CGRect, visibleCenterX: CGFloat) {
        
        let distanceFromCenter = visibleCenterX - self.center.x
        let absDistanceFromCenter = min(abs(distanceFromCenter), self.scalingOffset)
        let scale = absDistanceFromCenter * -1 / self.scalingOffset + 1
        
        frontConstraintY.constant = absDistanceFromCenter * (frontContainerView.bounds.size.height / 4) / self.scalingOffset + (-frontContainerView.bounds.size.height / 4)
        backConstraintY.constant = absDistanceFromCenter * (0 - (frontContainerView.bounds.size.height / 4 - yOffset / 2)) / self.scalingOffset + (frontContainerView.bounds.size.height / 4 - yOffset / 2)
        
        if let widthConstant = backContainerView.getConstraint(.Width) {
            widthConstant.constant = absDistanceFromCenter * (frontContainerView.bounds.size.width - (frontContainerView.bounds.size.width + yOffset)) / self.scalingOffset + (frontContainerView.bounds.size.width + yOffset)
        }
        
        if let heightConstant = backContainerView.getConstraint(.Height) {
            heightConstant.constant = absDistanceFromCenter * (frontContainerView.bounds.size.height - (frontContainerView.bounds.size.height + yOffset)) / self.scalingOffset + (frontContainerView.bounds.size.height + yOffset)
        }
